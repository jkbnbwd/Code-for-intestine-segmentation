...
def test_single_case(net1, net2, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                #print('test_patch shape = ', test_patch.shape)#torch.Size([1, 1, 16, 256, 256])
                #1/0

                with torch.no_grad():
                    media,tras_out = net1(test_patch.squeeze(0).reshape([16,1,256,256]))
                    tras_out =  torch.argmax(tras_out,dim = 1).unsqueeze(0).unsqueeze(0)
                    y1 = net2(torch.cat((test_patch,tras_out),1))
                    # ensemble
...
if __name__ == '__main__':
    ...
    model2 = net_factory_3d(net_type=args.model2, in_chns=2, class_num=2).cuda()
    ...
    for i in range(len(file_name_list)):
        ...
        print('model2 eval...... ')
        pred_np2 = test_single_case(
            model1, model2,  ct_np, stride_xy,stride_z,patch_size,n_labels)
        #print('pred_np2 shape = ',pred_np2.shape)#(506, 350, 350)
        #print('pred_np2 type = ',type(pred_np2))#<class 'numpy.ndarray'>
        #1/0
        ...
        if not os.path.exists('../result/miccai24/'+args.exp+'/model1'):
            os.makedirs('../result/miccai24/'+args.exp+'/model1')
            test1.to_csv('../result/miccai24/'+args.exp+'/model1/'+'model1_val.csv',encoding='gbk',mode = 'a')
            sitk.WriteImage(pred_img1, os.path.join('../result/miccai24/'+ args.exp+'/model1', 'test-'+data_path[0].split('/')[-1]))
            #sitk.WriteImage(pred_img1, './test.nii.gz')
        else:
            test1.to_csv('../result/miccai24/'+args.exp+'/model1/'+'model1_val.csv',encoding='gbk',mode = 'a',header=False)
            sitk.WriteImage(pred_img1, os.path.join('../result/miccai24/'+ args.exp+'/model1', 'test-'+data_path[0].split('/')[-1]))
            #sitk.WriteImage(pred_img1, './test.nii.gz')

        if not os.path.exists('../result/miccai24/'+args.exp+'/model2'):
            os.makedirs('../result/miccai24/'+args.exp+'/model2')
            test2.to_csv('../result/miccai24/'+args.exp+'/model2/'+'model2_val.csv',encoding='gbk',mode = 'a')
            sitk.WriteImage(pred_img2, os.path.join('../result/miccai24/'+ args.exp+'/model2', 'test-'+data_path[0].split('/')[-1]))
            #sitk.WriteImage(pred_img1, './test.nii.gz')
        else:
            test2.to_csv('../result/miccai24/'+args.exp+'/model2/'+'model2_val.csv',encoding='gbk',mode = 'a',header=False)
            sitk.WriteImage(pred_img2, os.path.join('../result/miccai24/'+ args.exp+'/model2', 'test-'+data_path[0].split('/')[-1]))
            #sitk.WriteImage(pred_img1, './test.nii.gz')